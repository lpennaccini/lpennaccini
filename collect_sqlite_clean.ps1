...
# Versione: 3.0 - SQLite Working Version (Clean)
...
try {
    ...
    if ($result) {
        ...
        Write-LogMessage "Statistiche database:" "INFO"
        ...
        if ($stats) {
            ...
            if ($stats.CriticalDisks -and $stats.CriticalDisks.Count -gt 0) {
                Write-LogMessage "ATTENZIONE: Dischi critici trovati: $($stats.CriticalDisks.Count)" "WARN"
            }
        }
    } else {
